	...
	SubShader
	{
		...
		Pass
		{
			...
			CBUFFER_START(UnityPerMaterial)
				...
				float4 _DistortionTiling1;
				float4 _DistortionSpeed;
				float _DissolveAmount;
				float4 _DissolveTiling1;
			...
			half4 frag(Varyings input) : SV_Target
			{
				...
				float2 distortionPanner = _Time.y * _DistortionSpeed.xy + uv * _DistortionTiling1.xy;
				...
				float2 dissolvePanner = _Time.y * _DissolveSpeed.xy + uv * _DissolveTiling1.xy;
				...
				half4 dissolve = pow(abs(SAMPLE_TEXTURE2D(_DissolveTexture, sampler_DissolveTexture, (distortedUv + float4(dissolvePanner, 0.0, 0.0)).xy)), _DissolveAmount.xxxx);
