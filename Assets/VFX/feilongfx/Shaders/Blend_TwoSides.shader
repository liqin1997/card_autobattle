Shader "Hovl/Particles/Blend_TwoSides"
{
	...
	SubShader
	{
		Tags { "RenderType"="TransparentCutout" "Queue"="Transparent" "IsEmissive"="true" "PreviewType"="Plane" "RenderPipeline"="UniversalPipeline" }
		Cull Off
		ZWrite Off
		...
		Pass
		{
			...
			half4 frag(Varyings input, FRONT_FACE_TYPE face : FRONT_FACE_SEMANTIC) : SV_Target
			{
				...
				float fresnel = pow(saturate(1.0 - dot(normalWS, viewDirWS)), max(_Fresnel, 0.0001));
				half4 fresnelFront = _FrontFacesColor * (1.0 - fresnel) + _FresnelEmission * _FresnelColor * fresnel;
				half4 frontColor = lerp(_FrontFacesColor, fresnelFront, saturate(_UseFresnel));
				half4 faceColor = IS_FRONT_VFACE(face, frontColor, _BackFacesColor);

				float2 mainUV = input.uv * _MainTex_ST.xy + _MainTex_ST.zw + _SpeedMainTexUVNoiseZW.xy * _Time.y;
				half4 mainTex = SAMPLE_TEXTURE2D(_MainTex, sampler_MainTex, mainUV);
				half4 normalEmission = faceColor * _Emission * input.color * input.color.a * mainTex;
				half4 separateEmission = (faceColor + _FresnelColor * mainTex * _SeparateEmission) * _Emission * input.color * input.color.a;

				float2 maskUV = input.uv * _Mask_ST.xy + _Mask_ST.zw;
				float2 noiseUV = input.customData.xy * _Noise_ST.xy + _Noise_ST.zw + _SpeedMainTexUVNoiseZW.zw * _Time.y + input.customData.w;
				half mask = SAMPLE_TEXTURE2D(_Mask, sampler_Mask, maskUV).r;
				half noise = SAMPLE_TEXTURE2D(_Noise, sampler_Noise, noiseUV).r;
				clip(mask * noise * lerp(1.0, input.customData.z, saturate(_UseCustomData)) - _Cutoff);
				return half4(lerp(normalEmission, separateEmission, saturate(_SeparateFresnel)).rgb, 1.0h);
			}
			ENDHLSL
		}
	}
